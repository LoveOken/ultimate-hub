...
const GAME_SEAT_REQUEST = function(will_sit) {
    "use strict";
...
const GAME_TO_CLIENT_INIT = function(
    do_once_connected,
    ...
    socket.on("update-finish", function(data) {
        ...
        let for_each_and_every_role = function(todo) {
            game.roles.forEach(function(role, index) {
                role.active.forEach(function(active, subindex) {
                    todo(role, index, active, subindex, function() {
